...
for i in tqdm(range(len(div)-1)):
    ...
    crouch = get_sentence_embeding(data["tweet"][start:end]) # BERTemius Crouch
    for j in range(768):
        diri[f"X{j+1}"] = np.array(crouch)[:, j]
    pd.DataFrame(diri).to_csv(f"../data/BERT/{batch_size}/data_{start}_{end-1}.csv", index=False)
    del crouch